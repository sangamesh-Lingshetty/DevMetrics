...
class GithubAnalyticsAPI {
  ...
  async getRepositoryPullRequests(owner, repo, options = {}) {
    try {
      const response = await this.client.get(`/github/repos/${owner}/${repo}/pulls`, {
        params: {
  ...
  async getRepositoryIssues(owner, repo, options = {}) {
    try {
      const response = await this.client.get(`/github/repos/${owner}/${repo}/issues`, {
        params: {
          state: options.state || 'all',
          limit: options.limit || 50
        }
  ...
  handleError(error) {
    if (error.response) {
      ...
      return new Error('No response from server. Please check if the backend is running and your connection.');
    } else {
